...
/**
 * ServiceDetail component following Clean Architecture and modern React practices
 */
const ServiceDetail = () => {
  const { serviceId } = useParams<{ serviceId: string }>();
  const { getServiceById } = useServices();
  
  // Use memoization to avoid recalculating the service unless serviceId changes
  const service = useMemo(() => getServiceById(serviceId || ''), [serviceId, getServiceById]);
  
  // Create a memoized WhatsApp link to avoid recreation on each render
  const whatsappLink = useMemo(() => {
    const message = encodeURIComponent(`Hola, me gustaría obtener más información sobre el servicio de ${service?.title || 'consultoría'}.`);
    return `https://wa.me/51992854449?text=${message}`;
  }, [service?.title]);
