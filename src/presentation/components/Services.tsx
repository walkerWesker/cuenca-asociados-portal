  ...
  return (
    ...
    >
      <div className="relative h-52 overflow-hidden">
        ...
        />
        <div className={`absolute inset-0 bg-gradient-to-t from-black/70 to-transparent transition-opacity duration-300 ${isActive ? 'opacity-80' : 'opacity-70'}`}></div>
        <div className="absolute bottom-4 left-4 flex items-center">
          <div className={`flex items-center justify-center w-12 h-12 rounded-lg bg-cuenca-blue text-white mr-3 transition-all duration-300 ${isActive ? 'scale-110 bg-cuenca-gold' : 'scale-100'}`}>
        ...
        <div className="space-y-3 mt-2">
          {service.details.slice(0, 2).map((detail, i) => (
            <li key={i} className="flex items-start list-none">
              <span className={`text-cuenca-blue mr-3 text-lg transition-all duration-300 ${isActive ? 'translate-x-1 text-cuenca-gold' : 'translate-x-0'}`}>•</span>
              <span className="text-gray-700">{detail}</span>
            </li>
          ))}
        </div>
        <div className={`mt-auto pt-4 font-medium transition-all duration-300 flex items-center ${isActive ? 'text-cuenca-gold translate-x-1' : 'text-cuenca-blue'}`}>
          Ver más detalles
          <span className={`ml-1 inline-block transition-transform duration-300 ${isActive ? 'translate-x-1' : 'translate-x-0'}`}>→</span>
        </div>
      </div>
    </Link>
  );
...
/**
 * Services component with modern React practices
 */
const Services = () => {
