                  ...
                  <ul className="text-sm text-gray-700 space-y-3 mb-5">
                    {service.details.slice(0, 2).map((detail, i) => (
                      <li key={i} className="flex items-start">
                        <span className={`text-cuenca-blue mr-2 transition-all duration-300 ${activeIndex === index ? 'translate-x-1 text-cuenca-gold' : 'translate-x-0'}`}>•</span>
                        <span>{detail}</span>
                      </li>
                    ))}
                  </ul>
                  <div className={`mt-auto font-medium transition-all duration-300 ${activeIndex === index ? 'text-cuenca-gold translate-x-1' : 'text-cuenca-blue'}`}>
                    Ver más detalles
